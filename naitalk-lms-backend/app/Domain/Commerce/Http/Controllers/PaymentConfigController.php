...
class PaymentConfigController extends Controller
{
    ...
    /** Client-owned mode: the organization's own Paystack/Flutterwave credentials. */
    public function setClientOwned(Request $request)
    {
        $data = $request->validate([
            ...
            'fee_bearer' => ['in:organization,learner,platform'],
        ...
        $config->fill([
            ...
            'fee_bearer' => $data['fee_bearer'] ?? 'organization',
     ...
     * for split settlement so the organization's share lands with them
     * directly — NAI TALK's commission is deducted at the provider level via
     * the split, not moved manually afterward.
     */
    ...
    private function present(?PaymentConfig $config): ?array
    {
        if (! $config) {
            return null;
        }
