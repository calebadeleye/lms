...
it('calculates the platform default 1% commission for managed-mode payments', function () {
    $config = PaymentConfig::create(['provider' => 'paystack', 'mode' => 'managed', 'fee_bearer' => 'organization']);
    ...
    expect($split['org_net_cents'])->toBe(10_000_00 - 10_000 - 150_00);
});

it('respects a commission percent override', function () {
    $config = PaymentConfig::create([
        'provider' => 'paystack', 'mode' => 'managed', 'fee_bearer' => 'organization', 'commission_percent' => 2.5,
    ]);
...
it('charges zero commission for client-owned mode — NAI TALK never touches that money', function () {
    $config = PaymentConfig::create(['provider' => 'paystack', 'mode' => 'client_owned', 'fee_bearer' => 'organization']);

    $split = app(CommissionService::class)->calculate($config, grossAmountCents: 10_000_00, providerFeeCents: 150_00);

    expect($split['commission_cents'])->toBe(0);
    expect($split['org_net_cents'])->toBe(10_000_00 - 150_00);
});

// --- Gateway modes -------------------------------------------------------

it('lets an owner connect their own client-owned Paystack credentials', function () {
    $owner = makeUserWithRole('owner');
    $token = $owner->createToken('t')->plainTextToken;

    $response = $this->putJson('/api/v1/admin/payment-config/client-owned', [
        'provider' => 'paystack',
        'public_key' => 'pk_test_abc123',
        'secret_key' => 'sk_test_supersecret',
        'webhook_secret' => 'whsec_abc',
        'fee_bearer' => 'organization',
...
it('lets an owner connect NAI TALK-managed payments and creates a provider subaccount', function () {
    Http::fake([
    ...
    $response = $this->putJson('/api/v1/admin/payment-config/managed', [
        'provider' => 'paystack',
...
it('reports a connected gateway when the provider returns a successful response', function () {
    ...
    $this->putJson('/api/v1/admin/payment-config/client-owned', [
        'provider' => 'paystack', 'public_key' => 'pk_test_abc', 'secret_key' => 'sk_test_abc',
        'webhook_secret' => 'whsec_abc', 'fee_bearer' => 'organization',
    ], ['Authorization' => "Bearer {$token}"])->assertOk();

    $this->postJson('/api/v1/admin/payment-config/test-connection', [], [
...
it('reports a disconnected gateway for invalid credentials, even on a non-401 error status', function () {
    ...
    $this->putJson('/api/v1/admin/payment-config/client-owned', [
        ...
        'webhook_secret' => 'whsec_bad', 'fee_bearer' => 'organization',
    ], ['Authorization' => "Bearer {$token}"])->assertOk();
...
it('verifies a webhook, fulfills the order exactly once, and computes the 1% commission', function () {
    ...
    expect($payment->org_net_cents)->toBe(500000 - 5000 - 7500);
...
it('lets a finance manager issue a refund', function () {
    ...
    $payment = Payment::create([
        ...
        'gross_amount_cents' => 200000, 'currency' => 'NGN', 'fee_bearer' => 'organization', 'paid_at' => now(),
    ]);
