...
it('unlocks lessons and reports is_enrolled on the public course detail page for a logged-in enrolled student', function () {
    $this->postJson("/api/v1/courses/{$this->course->id}/enrol", [], [
        'Authorization' => "Bearer {$this->studentToken}",
    ])->assertCreated();

    $response = $this->getJson("/api/v1/courses/{$this->course->slug}", [
        'Authorization' => "Bearer {$this->studentToken}",
    ])->assertOk();
...
it('still shows a published course to an anonymous visitor, with non-preview lessons locked', function () {
    $response = $this->getJson("/api/v1/courses/{$this->course->slug}")->assertOk();
...
it('hides a category with no published courses from the public catalogue sidebar, but not from admin category management', function () {
    ...
    $public = $this->getJson('/api/v1/course-categories?only_with_published=1')->assertOk();
    expect(collect($public->json('data'))->pluck('name'))->toContain('HR')->not->toContain('Empty Category');

    $admin = $this->getJson('/api/v1/course-categories')->assertOk();
...
it('refuses to enrol in a paid course — checkout is Phase 3', function () {
    ...
    $this->postJson("/api/v1/courses/{$paidCourse->id}/enrol", [], [
        'Authorization' => "Bearer {$this->studentToken}",
    ])->assertStatus(422);
});
...
it('clamps a client-submitted video position to the lesson duration and marks complete at 90%', function () {
    ...
    $response = $this->postJson("/api/v1/lessons/{$this->videoLesson->id}/progress/position", [
...
it('lets a student manually mark an externally-embedded video lesson complete, since its playback position cannot be tracked', function () {
    ...
    $response = $this->postJson("/api/v1/lessons/{$this->videoLesson->id}/progress/complete", [], [
        'Authorization' => "Bearer {$this->studentToken}",
    ])->assertOk();
...
it('refuses to manually mark a directly-hosted video lesson complete — that only completes via watched position', function () {
    ...
    $this->postJson("/api/v1/lessons/{$this->videoLesson->id}/progress/complete", [], [
        'Authorization' => "Bearer {$this->studentToken}",
    ])->assertStatus(500);
});
...
it('computes course completion percentage across mandatory lessons and completes the enrolment at 100%', function () {
    ...
    $this->postJson("/api/v1/lessons/{$this->videoLesson->id}/progress/position", [
        'position_seconds' => 100,
    ], ['Authorization' => "Bearer {$this->studentToken}"])->assertOk();

    $response = $this->postJson("/api/v1/lessons/{$this->readingLesson->id}/progress/complete", [], [
        'Authorization' => "Bearer {$this->studentToken}",
    ])->assertOk();
...
it('auto-grades a quiz and marks the quiz lesson complete on a passing attempt', function () {
    ...
    $attempt = $this->postJson("/api/v1/lessons/{$quizLesson->id}/quiz/attempts", [], [
        'Authorization' => "Bearer {$this->studentToken}",
    ])->assertCreated();

    $result = $this->postJson("/api/v1/quiz-attempts/{$attempt->json('data.id')}/submit", [
...
it('lets a content manager create and publish a course', function () {
    $admin = makeUserWithRole('content-manager');
    ...
    $this->postJson("/api/v1/admin/courses/{$created->json('data.id')}/publish", [], [
...
it('lets a content manager upload a material to a file-type lesson', function () {
    ...
    $response = $this->postJson("/api/v1/admin/lessons/{$fileLesson->id}/material", ['file' => $file], [
        'Authorization' => "Bearer {$token}",
    ])->assertOk();
...
it('lets an enrolled student download a lesson material but blocks a non-enrolled one', function () {
    ...
    $file = \Illuminate\Http\UploadedFile::fake()->create('handbook.pdf', 500, 'application/pdf');
    $this->postJson("/api/v1/admin/lessons/{$fileLesson->id}/material", ['file' => $file], [
        'Authorization' => "Bearer {$adminToken}",
    ])->assertOk();

    // Sanctum's RequestGuard caches the resolved user across HTTP calls
    // within one test — without this, the check below would keep resolving
    // the content manager from the upload call above instead of the
    // student's own token.
    $this->app->make('auth')->forgetGuards();

    // Not enrolled yet — blocked.
    $this->getJson("/api/v1/lessons/{$fileLesson->id}/material", [
        'Authorization' => "Bearer {$this->studentToken}",
    ])->assertStatus(403);
    ...
    $this->get("/api/v1/lessons/{$fileLesson->id}/material", [
        'Authorization' => "Bearer {$this->studentToken}",
    ])->assertOk();
});
